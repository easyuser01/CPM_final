                    ...
                    if let criteriaDataArray = getCriteriaData(for: atype) {
                        VStack {
                            ForEach(criteriaDataArray, id: \.self) { criteriaData in
                                Text("atype: \(atype)")
                                Text("눈: \(criteriaData.snow)")
                                Text("강수량: \(criteriaData.rainfall)")
                                Text("최저 기온: \(criteriaData.temperaturemin)")
                                Text("최고 기온: \(criteriaData.temperaturemax)")
                                Text("풍속: \(criteriaData.wind)")
                            }
                        }
                        .padding()
                    } else {
                        Text("\(atype)에 대한 데이터를 찾을 수 없습니다.")
                    }
                }
            }
            .font(.headline)
